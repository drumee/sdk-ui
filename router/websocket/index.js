...
function shouldSkip(keys, model, target) {
  let src = model.__oldItem;
  let dest = model.__newItem;
  if (_.isArray(keys)) {
    if (src && dest) {
      for (var k of keys) {
        if (src[k] !== target.mget(k) && dest[k] !== target.mget(k))
          return true;
      }
    } else {
      for (var k of keys) {
        if (model[k] !== target.mget(k)) return true;
      }
    }
    ...
    for (var k in keys) {
      let v = keys[k];
      if (src && dest) {
        if (src[k] !== target.mget(v) && dest[k] !== target.mget(v))
          return true;
      } else {
        if (model[k] !== target.mget(v)) return true;
      }
    }
  }
  return false;
...
function __processMessage__(msg = {}) {
  ...
  this.gossip(`AAA:52 [processMessage] service=${service}`, options, model);
  if (this.onWsMessage) {
    return this.onWsMessage(service, model, options);
  }
  // if (options && this.onLiveUpdate) {
  //   //this.debug(`AAA:37 [swRouter] service=${service}`, options, model);
  //   let keys = options.keys;
  //   if (keys == "*") {
  //     service = options.service || payload.service;
  //     this.onLiveUpdate(service, model, options);
  //     return;
  //   }
  //   if (options && options.route && _.isArray(model)) {
  //     //this.debug(`AAA:444 service=${service}`, options, model);
  //     for (var m of model) {
  //       if (shouldSkip(keys, m, this)) continue;
  //       service = options.service || payload.service;
  //       this.onLiveUpdate(service, m, options);
  //     }
  //     return;
  //   }

  //   if (shouldSkip(keys, model, this)) return;
  //   service = options.service || payload.service;
  //   this.onLiveUpdate(service, model, options);
  //   return;
  // }
...
class __router_websocket extends LetcBox {
  initialize(opt) {
    ...
    // window.addEventListener('online', this.onLinkUp.bind(this));
    // window.addEventListener('offline', this.onLinkDown.bind(this));
