 ...
 * 
 */
class ___widget_chat extends LetcBox {
  constructor(...args) {
    super(...args);
 ...
 */
  ...
  onUiEvent(cmd, args = {}) {
    ...
    switch (service) {
      ...
      case _e.upload:
        ...
        return this.__message.showSend();
  ...
  handleReceivedMsg(data) {
    ...
    if (data.echoId && this.echoId == data.echoId) {
      messageArr = this.__list.getItemsByAttr('echoId', data.echoId);
      if (messageArr.length) {
        delete data.echoId;
        data.kind = 'widget_chat_item';
        data.logicalParent = this;
        data.uiHandler = this;
        messageArr[0].respawn(data);
        this.echoId = null;
      }
    } else {
      messageArr = this.__list.getItemsByAttr('message_id', data.message_id)
      if (!messageArr.length) {
        data.kind = 'widget_chat_item';
        data.logicalParent = this;
        data.uiHandler = this;
        this._newMsgCount++;
        this.__buttonScroll.el.dataset.count = this._newMsgCount;
        const node = this.__list.append(data);
      }
    }
...
___widget_chat.initClass();

module.exports = ___widget_chat;
